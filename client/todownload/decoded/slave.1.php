...
if ($inivar[virt] = 'xen') {    		digbbcajef(); }
if ($inivar[virt] = 'kvm') {			bbcgdeiced(); }
...
if ($inivar[virt] = 'xen') {
    echo 'Edit /boot/grub/grub.conf and make sure the server is set to boot into the Xen kernel.';
    echo 'Important!! Please read the following: https://documentation.solusvm.com/display/DOCS/Xen+XL+Setup';
    echo 'Run this command once rebooted: php /usr/local/solusvm/includes/xenkernel.php .';
    echo 'Please set SELINUX=disabled in /etc/selinux/config before rebooting.';
}


if ($inivar[virt] = 'kvm') {
    echo 'IMPORTANT!! You need to setup a network bridge before you can use KVM on this server.';
