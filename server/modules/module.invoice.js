...
function balanceDatesCheck(refObj){
	...
	toUpdate.lastBalanceCheck = refObj.user.lastBalanceCheck || moment().toDate();
	toUpdate.accountBalance = parseFloat(refObj.accountBalance).toFixed(2);
	if(parseFloat(refObj.accountBalance) <= 0) toUpdate.numberofRemindersSent = 0;
...
function decideBalance(refObj){
	...
	if(refObj.shouldWeSend) console.log(refObj.userid, refObj.user.accountBalance, refObj.user.reminderStat, refObj.shouldWeSend);
	deferred.resolve(refObj);
	return deferred.promise;
}
...
function setupMailObject(mObject){
	...
	mObject.subject = mObject.settings.companyname+' Invoice for '+moment(mObject.invoice.details.date).format('DD MMM YYYY')+ ' (Invoice: '+ mObject.invoiceID +')';
