...
function persistentInitiateVNCProxy(host, port, localport, tools){
	...
	var curCommand = path.resolve('.') + '/client/lib/no-vnc/utils/websockify '+localport+' '+ host +':'+ port +' --cert /etc/nginx/ssl/www_epmvirtual_com.pem --ssl-only -D --run-once --timeout=60 >> /tmp/websockify.log';
