...
function persistentInitiateVNCProxy(host, port, localport, tools){
	var deferred = Q.defer();
	var curCommand = path.resolve('.') + '/client/lib/no-vnc/utils/websockify '+localport+' '+ host +':'+ port +' --cert /etc/nginx/ssl/www.epmvirtual.com.fullchain.pem --key /etc/nginx/ssl/www.epmvirtual.com.privkey.pem --ssl-only -D --run-once --timeout=60';
