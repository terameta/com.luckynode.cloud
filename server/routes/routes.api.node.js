			...
			db.nodes.findOne({_id:mongojs.ObjectId(req.params.id)}, function(nodeerr, nodedata){
				if(nodeerr){
					...
					if(!curHost){
						...
						var whoamitext = '{ \\\"whoami\\\":\\\"' + nodedata.name + '\\\", \\\"whoamid\\\":\\\"' + nodedata._id.toString() + '\\\"}';
						...
						steps.push({ order: steps.length,	Description: 'Install DHCP & Apparmor',		command: 'apt-get -y install isc-dhcp-server apparmor apparmor-profiles >> /tmp/install.log 2>&1' 			});
